...
export ETCD_MEASURE_THR_ENABLED=true
export ETCD_MEASURE_THR_FILENAME=/tmp/throughput-${nodeID}.out
...
~/go/src/github.com/Lz-Gustavo/etcd/bin/etcd --name=${nodeID} \
  --log-level=debug \
  --initial-advertise-peer-urls http://${nodeIP}:2381 \
  --listen-peer-urls=http://${nodeIP}:2381 \
  --listen-client-urls=http://${nodeIP}:2371 \
  --advertise-client-urls=http://${nodeIP}:2371 \
